...
def move_rope_knot(front_knot, curr_knot):
    fx, fy = front_knot
    cx, cy = curr_knot

    cx = (fx + cx) // 2 if fy == cy or abs(fx - cx) > 1 else fx
    cy = (fy + cy) // 2 if fx == cx or abs(fy - cy) > 1 else fy

    return cx, cy


def move_rope_head(direction, times):
    while times:
        times -= 1
        move = MOTION_GRID[direction]
        head_coo = move(*knots[HEAD])
        knots[HEAD] = head_coo
        head_paths.add(head_coo)

        for idx in range(KNOTS_COUNT - 1):
            curr_pos = idx + 1
            front = knots[idx]
            curr = knots[curr_pos]
            if not is_front_knot_nearby(front, curr):
                coo = move_rope_knot(front, curr)
                knots[curr_pos] = coo
...
def is_front_knot_nearby(front_knot, curr_knot):
    x, y = curr_knot
    u, d, l, r = y+1, y-1, x-1, x+1

    return front_knot in (
        (l, u), (x, u), (r, u),
        (l, y), (x, y), (r, y),
        (l, d), (x, d), (r, d),
    )
...
def print_grid(stream=None):
    left_edge = max(x for x, _ in head_paths)
    right_edge = min(x for x, _ in head_paths)
    top_edge = max(y for _, y in head_paths)
    bottom_edge = min(y for _, y in head_paths)

    length = max(abs(left_edge - right_edge) + 1, 4)
    height = max(abs(top_edge - bottom_edge) + 1, 4)

    grid = []
    for y in range(height + 1):
        line = '.' * length if y != top_edge else '─' * length

        zero_x = right_edge * -1
        line = line[:zero_x] + '|' + line[zero_x+1:]
        grid.append(line)

    def set_knot_in_grid(x, y, symbol='#'):
        x_offset = x + (right_edge * -1)
        y_offset = top_edge - y
        line = grid[y_offset]
        grid[y_offset] = line[:x_offset] + symbol + line[x_offset+1:]

    set_knot_in_grid(0, 0, '0')
    for x, y in tail_paths:
        sym = 't'
        set_knot_in_grid(x, y, sym)
...
if __name__ == '__main__':
    start_pos = (0, 0) 
    knots = [start_pos] * KNOTS_COUNT
    tail_paths = set((start_pos,))
    head_paths = set((start_pos,))

    with open('inputs/9_rope_motions.txt') as file:
        for line in file:
            motion = line.strip()
            direction, times = motion.split()
            move_rope_head(direction, int(times))

    head_x, head_y = knots[HEAD]
    tail_x, tail_y = knots[TAIL]
    print(f"The head finish x={head_x}, y={head_y}")
    print(f"The tail finish x={tail_x}, y={tail_y}")
    print(f"The tail visits {len(tail_paths)} positions at least once")
