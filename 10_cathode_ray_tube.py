...
def draw_on_CRT():
    draw_position = cycle - 1
    row, pixel = divmod(draw_position, CRT_WIDTH)
    if pixel in range(sprite_middle - 1, sprite_middle + 2):
        crt_screen[row][pixel] = LIT_PIXEL
...
if __name__ == '__main__':
    cycle = 0
    register_X = 1
    sprite_middle = register_X
    crt_screen = [['.'] * CRT_WIDTH for _ in range(CRT_HEIGHT + 1)]
    signal_strength_sum = 0

    with open('inputs/10_program_instructions.txt') as file:
        for line in file:
            cmd, value = parse_instruction(line.strip())
            cycles_to_complete = CMD_TO_CYCLES[cmd]
            while cycles_to_complete:
                cycle = cycle + 1
    ...
    print(f'{cycle} cycles was performed')
    print(f'Register X is {register_X}')
    print(f'The sum of important signal strengths is {signal_strength_sum}')
    print_crt()
...
then the row below that, and so on. The left-most pixel in each row is in position 0, and the right-most pixel in each row is in position 39.
